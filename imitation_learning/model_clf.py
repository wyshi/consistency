...
from imitation_learning.utils import print_cm
import tqdm
...
from imitation_learning.GPTModel import GPT2LMHeadModel_modified
from pytorch_pretrained_bert import OpenAIAdam
import imitation_learning.config as cfg
from torch.nn import Identity
from imitation_learning.utils import is_repetition_with_context

from imitation_learning.KnowledgeBase.KB import HumanRule
from imitation_learning.KnowledgeBase import KB
...
class ModelClassifier(object):
    def __init__(self, config, which_to_train, model_A, model_B, tokenizer, device1, device2, clf_A=None, clf_B=None, clf_TF=None):
        ...
        # super().__init__()
        self.config = config
#        self.le_A = load_pkl("../imitation_learning/data/labelencoder_A.pkl")
#        self.le_B = load_pkl("../imitation_learning/data/labelencoder_B.pkl")
        # self.le_A = load_pkl(os.path.join(os.getcwd(), "imitation_learning/data/labelencoder_A.pkl"))
        # self.le_B = load_pkl(os.path.join(os.getcwd(), "imitation_learning/data/labelencoder_B.pkl"))
        try:
            self.le_A = load_pkl(os.path.join(os.getcwd(), "imitation_learning/data/labelencoder_A.pkl"))
        except:
            self.le_A = load_pkl("../imitation_learning/data/labelencoder_A.pkl")
    ...
    def load_data(self):
        # load training data
        try:
            self.train_data = load_pkl("../imitation_learning/data/train_data.pkl")
            self.val_data = load_pkl("../imitation_learning/data/val_data.pkl")
            demo_train = torch.load("../imitation_learning/demonstration/old_model/demonstration_train_with_text_only.pkl", map_location="cpu")
            demo_val = torch.load("../imitation_learning/demonstration/old_model/demonstration_val_with_text_only.pkl", map_location="cpu")
        except:
            self.train_data = load_pkl(os.path.join(os.getcwd(), "imitation_learning/data/train_data.pkl"))
            self.val_data = load_pkl(os.path.join(os.getcwd(), "imitation_learning/data/val_data.pkl"))
            demo_train = torch.load(os.path.join(os.getcwd(), "imitation_learning/demonstration/old_model/demonstration_train_with_text_only.pkl"), map_location="cpu")
            demo_val = torch.load(os.path.join(os.getcwd(), "imitation_learning/demonstration/old_model/demonstration_val_with_text_only.pkl"), map_location="cpu")
                
        self.train_data_TF, self.val_data_TF = demo_train, demo_val
    ...
    def set_one_sent_past(self, sent, which_role, past):
    ...
    def set_past(self, context):
        past = None
        for role, sent in context:
            past = self.set_one_sent_past(sent=sent, which_role=role, past=past)
        return past

    def predict(self, separate_sents, which_task, context=None, past=None):
        "separate_sents: list of sentences with no prefix"
        
        if past is None:
            # need to rebuild the history from context
            assert context is not None
            past = self.set_past(context)

        if type(separate_sents) is not list and type(separate_sents) is str:
            separate_sents = [separate_sents]
