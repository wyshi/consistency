...
def calculate_num_success_candidates_for_eval_set(bot, dataloader, mode):
    ...
    while dial_i < MAX_DIALOGS:
        try:
            ...
            if result is not None:
                TOTAL_TURNS += 1
                response, [sents_success, sents_failed], have_enough_candidates, usr_input_text, sent_act_candidates = result
                # import pdb
                # pdb.set_trace()
                TOTAL_SUCCESS_CANDIDATES += len(sents_success)
            if CurrentModelConfig.candidate_select_strategy != cfg.HUMAN_SELECTION:
...
def validate(dataloader, model_A, model_B, ep=0):
    ...
    with torch.no_grad():
        ...
        with open("Eval/simulated_dialogs.txt", "a") as fh:
            ...
            print(f"Epoch {ep} Validation Perplexity all: {np.mean(total_ppl)} Variance: {np.var(total_ppl)}")
            ...
            fh.write(f"Epoch {ep} Validation Perplexity all: {np.mean(total_ppl)} Variance: {np.var(total_ppl)}\n")
                    ...
                    for role_id, dial_turn_inputs, dial_sent in zip(role_ids, dial_inputs, dial_sents):
                        ...
                        if role_id == 0:
                            ...
                            for sent, reward in zip([ground_truth] + sents_success + sents_failed, cur_rewards):
                                ...
                                dialog_context_ids.append(f"{self.dialog_i}-{self.turn_i}-supervised")
...
if PPL_ONLY:
    ...
    if __name__ == "__main__":
        ...
        flat_final_targets_A, flat_final_responses_A, flat_final_targets_B, flat_final_responses_B = flatten_list(final_targets_A),\
            flatten_list(final_responses_A), flatten_list(final_targets_B), flatten_list(final_responses_B)
        automatic_metric(flat_final_targets_A, flat_final_responses_A, flat_final_targets_B, flat_final_responses_B)